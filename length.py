...
def yuanzhu_length(name):
    ...
    print("原始数据的shape：", data.shape)
    for i in range(data.shape[0]):
        if str(data[i, 3]) == "0.000000000000000000e+00":
            data_gound.append([points[i, 0], points[i, 1], points[i, 2]])
        if str(data[i, 3]) == "2.000000000000000000e+00":
            data_ding.append([points[i, 0], points[i, 1], points[i, 2]])
        if str(data[i, 3]) == "1.000000000000000000e+00":
            data_ce.append([points[i, 0], points[i, 1], points[i, 2]])
    # 得到底面和侧面的点
    data_ding = np.array(data_ding)
    data_ce = np.array(data_ce)
    data_gound = np.array(data_gound)
    print("顶部数据的shape：", data_ding.shape)
    print("侧部数据的shape：", data_ce.shape)
    print("地面数据的shape：", data_gound.shape)

    # 找顶部边缘最小距离
    ding_data_average = np.average(data_ding, axis=0)
    point1 = max_length_point(ding_data_average, data_ding)
    point2 = min_length_point(point1, data_gound)
    heigh = np.linalg.norm(point1 - point2)
    print("顶部边缘最小距离点：", point1, point2)
    print("标定物的高度：", heigh)
    ...
    np.savetxt("E:/python_file/anode_copper_ranging/res/yuanzhu_point.txt", points, delimiter=' ', fmt='%s')
    return heigh
